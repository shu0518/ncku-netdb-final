...
def check_pairwise_role(rep: Report) -> None:
    ...
    text = _read(p)
    entries = re.findall(
        r"^\s*-\s*role:\s*(code-author|bug-hunter)\s*\n\s*skill_path:\s*(\S+)\s*$",
        text,
        re.MULTILINE,
    )
    roles = {role: path.rstrip("/") for role, path in entries}
    expected = {"code-author", "bug-hunter"}
    if set(roles) != expected:
        rep.add("pairwise-role:roles", False,
                f"expected roles {sorted(expected)}, got {sorted(roles)}")
        return
    rep.add("pairwise-role:roles", True, ", ".join(sorted(roles)))
    missing_paths = []
    for role, sp in sorted(roles.items()):
        target = REPO_ROOT / sp
        if not target.is_dir():
            missing_paths.append(f"{role}: {sp}")
    rep.add("pairwise-role:skill_paths", not missing_paths,
            "" if not missing_paths else f"path not a directory: {missing_paths}")
